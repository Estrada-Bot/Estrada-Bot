...
class VerifyView(discord.ui.View):
    def __init__(self):
        ...
        verify_emoji = emoji_loader.get_obj("7490modernverify") or panel_settings.get("verify_button_emoji")
    ...
    async def verify_button(self, interaction: discord.Interaction):
        e_check    = emoji_loader.get("60226check")    or "✅"
        e_x        = emoji_loader.get("8118xmark")     or "❌"
        e_warning  = emoji_loader.get("11838warning")  or "⚠️"
        e_verified = emoji_loader.get("78934verified") or "✅"
        ...
        try:
            ...
            embed.set_footer(
                text=panel_settings.render(panel_settings.get("verify_footer")) or f"{config.SERVER_NAME}"
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
