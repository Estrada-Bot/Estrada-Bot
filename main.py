...
class EstradaBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        #
        # NOTE: each view is imported/registered in its own try/except so a
        # single missing or broken cog can't silently prevent the *other*
        # views from being re-attached after a restart (that used to happen
        # here — one shared try/except around every import meant that if
        # any one import failed, none of the buttons below it got
        # re-registered, and they'd stop responding until the bot reloaded
        # the message that held them).
        try:
            from cogs.tickets import TicketCreateView, TicketControlView
            self.add_view(TicketCreateView())
            self.add_view(TicketControlView())
        except Exception as e:
            print(f'⚠️ Ticket views: {e}')

        try:
            from cogs.verify import VerifyView
            self.add_view(VerifyView())
        except Exception as e:
            print(f'⚠️ Verify view: {e}')

        try:
            from cogs.create_voice import VoiceControlView
            self.add_view(VoiceControlView())
        except Exception as e:
            print(f'⚠️ Voice control view: {e}')
...
# ── Emojis used across the /help menu ────────────────────────────────────
# Plain unicode by default (safe — never breaks the menu). Each section also
# carries "app_emoji": the Estrada Application Emoji name to try instead,
# resolved at runtime via cogs.emoji_loader — see _section_emoji() below.
HELP_SECTIONS = [
    {
        "key": "admin",
...
def _section_emoji(section: dict):
    """Real Estrada emoji if it's loaded under section['app_emoji'],
    otherwise the safe unicode default in section['emoji']."""
    return emoji_loader.get_obj(section["app_emoji"]) or section["emoji"]
...
class HelpLayoutView(discord.ui.LayoutView):
    ...
    def __init__(self, active_key: str | None = None):
        ...
        header_text = (
            f"**{emoji_loader.get('38083rules') or '📖'} {config.BOT_NAME} — Commands**\n"
        ...
        if section:
            body_text = f"**{_section_emoji(section)} {section['title']}**\n{section['value']}"
        else:
            body_text = f"اختار قسم من المنيو تحت باش تشوف الأوامر ديالو {emoji_loader.get('34996chercher') or '🔍'}"

        footer_text = f"-# {config.SERVER_NAME}"
...
async def help_cmd(interaction: discord.Interaction):
    # Defensive: if the Application Emoji cache is somehow still empty at
    # this point (e.g. on_ready's load raced with this command on a fresh
    # restart), refresh it right now instead of showing unicode fallbacks.
    if not emoji_loader._emoji_cache:
        await emoji_loader.load_emojis(bot)
...
async def random_choice(interaction: discord.Interaction, choices: str = ""):
    ...
    if not choices.strip():
        ...
        embed.set_footer(
            text=f"طلبه: {interaction.user.display_name} | {config.SERVER_NAME}",
