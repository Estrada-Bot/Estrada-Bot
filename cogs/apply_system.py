"""
Unified Application System —Estrada 
Professional Estrada-style apply panels.
...
class ApplyButtonView(discord.ui.LayoutView):
    ...
    def __init__(self, kind: str, cfg: dict):
        ...
        # footer line inside the card, exactly like the Estrada panel.
...
class ApplyReviewView(discord.ui.View):
    ...
    async def btn_accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        pending_role_id = cfg.get("pending_role_id")
        role_id = cfg.get("role_id")

        # ── Two-stage flow: if a pending (voice test) role is configured,
        # give THAT one now — the final whitelist role only gets given
        # after staff click "Voice Test Passed" below. ────────────────────
        uses_voice_test = bool(pending_role_id)
        role_to_give_id = pending_role_id if uses_voice_test else role_id
        role_given = False
        if role_to_give_id:
        ...
        decision_value = f"Accepted by: {interaction.user.mention}\n<t:{int(datetime.now().timestamp())}:F>\n"
        if uses_voice_test:
            decision_value += "🎙️ Pending role given — waiting for Voice Test" if role_given else "⚠️ Could not give the pending role"
        else:
            decision_value += "Role given" if role_given else "Could not give the role"
        new_embed.add_field(name="Decision", value=decision_value, inline=False)

        if uses_voice_test:
            # Swap to the follow-up view: a single "Voice Test Passed" button.
            follow_up_view = VoiceTestPassView(kind=kind, applicant_id=uid)
            await interaction.message.edit(embed=new_embed, view=follow_up_view)
        else:
            for c in self.children:
                c.disabled = True
        ...
        try:
            if uses_voice_test:
                card = DecisionCardView(
                    icon="🎙️",
                    heading=f"{KIND_TITLE.get(kind, kind.title())} Application — Accepted, Voice Test Next",
                    body_lines=[
                        f"Your {KIND_TITLE.get(kind, kind).lower()} application has been approved by {interaction.user.mention}!",
                        f"• **Server** : {interaction.guild.name}",
                        "• **Next step** : join the voice test channel whenever staff are ready for you.",
                    ] + ([] if role_given else ["• ⚠️ The pending role could not be assigned automatically — contact an administrator."]),
        ...
        await interaction.response.send_message(
            f"✅ Accepted {member.mention}{', pending role given — do the voice test next' if uses_voice_test and role_given else (', role given' if role_given else '')}.",
            ephemeral=True
        )
...
class VoiceTestPassView(discord.ui.View):
    """The one-button follow-up shown on a review message after Accept, when
    the kind has a `pending_role_id` configured (two-stage whitelist flow).
    Staff click this once the applicant has finished their voice test — it
    swaps the pending role for the final whitelist role."""

    def __init__(self, kind: str = "whitelist", applicant_id: int = 0):
        super().__init__(timeout=None)
        self.kind = kind
        self.applicant_id = applicant_id
        self.btn_pass.custom_id = f"apply_voicepass_{kind}_{applicant_id}"

    def _parse_ids(self) -> tuple[str, int]:
        try:
            parts = self.btn_pass.custom_id.split("_")
            return parts[2], int(parts[3])
        except Exception:
            return self.kind, self.applicant_id
    ...
    @discord.ui.button(label="🎙️ Voice Test Passed", style=discord.ButtonStyle.success, custom_id="apply_voicepass_0_0")
    async def btn_pass(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        cfg = get_kind_cfg(interaction.guild_id, kind)
        pending_role = interaction.guild.get_role(cfg.get("pending_role_id") or 0)
        final_role = interaction.guild.get_role(cfg.get("role_id") or 0)

        if pending_role and pending_role in member.roles:
            try:
                await member.remove_roles(pending_role, reason=f"Voice test passed — confirmed by {interaction.user}")
            except discord.Forbidden:
                pass
        ...
        if final_role:
            try:
                await member.add_roles(final_role, reason=f"Voice test passed — confirmed by {interaction.user}")
                role_given = True
            except discord.Forbidden:
                pass
        ...
        old = interaction.message.embeds[0]
        new_embed = old.copy()
        new_embed.title = f"{old.title} — Voice Test Passed"
        new_embed.add_field(
            name="Voice Test",
            value=(
                f"Confirmed by: {interaction.user.mention}\n"
                f"<t:{int(datetime.now().timestamp())}:F>\n"
                f"{'✅ Whitelist role given' if role_given else '⚠️ Could not give the whitelist role'}"
            ),
            inline=False,
        )
        ...
        try:
            card = DecisionCardView(
                icon="🏁",
                heading=f"{KIND_TITLE.get(kind, kind.title())} — Voice Test Passed!",
                body_lines=[
                    f"You passed the voice test, confirmed by {interaction.user.mention}. Welcome to the whitelist! 🎉",
                    f"• **Server** : {interaction.guild.name}",
                ] + ([] if role_given else ["• ⚠️ The whitelist role could not be assigned automatically — contact an administrator."]),
                accent_color=config.SUCCESS_COLOR,
            )
            await member.send(view=card)
        except discord.Forbidden:
        ...
        await interaction.response.send_message(
            f"✅ {member.mention} passed the voice test{', whitelist role given' if role_given else ''}.",
            ephemeral=True
        )
...
class SetupApplyControlView(discord.ui.View):
    ...
    async def post_panel(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        cfg = {
            ...
            "pending_role_id": session.get("pending_role_id"),
...
class TitleModal(discord.ui.Modal, title="📝 Set Title, Description & Notice"):
    title_field = discord.ui.TextInput(
        label="Panel title",
        placeholder="e.g. Estrada — Staff Application",
        max_length=100,
        required=False,
    )
...
class ButtonModal(discord.ui.Modal, title="🎛️ Customize Button Text"):
    ...
    footer_field = discord.ui.TextInput(
        ...
        style=discord.TextStyle.paragraph,
        placeholder="With love, Estrada Team.",
        max_length=200,
        required=False,
    )
...
class ApplySystem(commands.Cog):
    ...
    @app_commands.describe(
        ...
        pending_role="Temporary role given right after Accept, before the voice test — final whitelist role is given only after Voice Test Passed (optional)",
    ...
    async def setup_apply(
        ...
        pending_role: discord.Role = None,
        banner_url: str = None,
    ):
        ...
        _sessions[interaction.user.id] = {
            ...
            "pending_role_id": pending_role.id if pending_role else existing.get("pending_role_id"),
        ...
        pending_line = ""
        pending_id_final = _sessions[interaction.user.id]["pending_role_id"]
        if pending_id_final:
            pending_role_obj = interaction.guild.get_role(pending_id_final)
            if pending_role_obj:
                pending_line = f"🎙️ Pending (voice test) role: {pending_role_obj.mention}\n"
