"""
Subscribe Approval System — Estrada
─────────────────────────────────────────────
...
# ─── custom emojis — safe unicode defaults (real Estrada emojis are
# resolved dynamically via emoji_loader where used below; these are the
# fallback if the matching Application Emoji isn't loaded) ──────────────────
EMOJI = {
    "accept": "✅",
    "reject": "❌",
    "proof": "✅",
...
class SubscribeReviewPanel(discord.ui.View):
    def __init__(self, target_user_id: int, guild_id: int, subscriber_role_id: int, reviewer_role_id: int = None):
        ...
        accept_emoji = emoji_loader.get_obj("60226check") or panel_settings.get("subscribe_accept_emoji") or EMOJI["accept"]
        reject_emoji = emoji_loader.get_obj("8118xmark")  or panel_settings.get("subscribe_reject_emoji") or EMOJI["reject"]
