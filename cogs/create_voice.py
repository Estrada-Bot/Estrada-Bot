...
Architecture (v2 — single shared panel):
  ...
  embed/button uses the real Estrada custom emojis.
...
# ─── Emojis ──────────────────────────────────────────────────────────────────
# Plain unicode by default — safe at import time (buttons are decorated
# before the bot has connected, so no custom Application Emoji can be
# resolved yet). Each button's real Estrada custom emoji (if one exists
# under the matching name in Application Emojis) is applied on top of this
# in VoiceControlView.__init__, once the bot is actually running — see
# _APP_EMOJI_NAMES below. If the custom emoji isn't found, these unicode
# ones stay as a safe fallback instead of crashing with "Invalid emoji".
EMOJI = {
    "voice":    "🔊",
    "owner":    "👑",
    "members":  "👥",
...
# Button key → real Application Emoji name (from Developer Portal → Emojis)
# to try loading at runtime — see above.
_APP_EMOJI_NAMES = {
    "lock": "80091fermer", "unlock": "9068ouvert", "hide": "300018dmids",
    "show": "476439dmids", "limit": "19492membres", "invite": "50494lien",
    "permit": "85722ajouter", "rename": "36438designer",
    "bitrate": "26295bolt", "region": "64005web", "template": "62470logs",
    "claim": "747328ownericon", "transfer": "8997modernrefresh",
    "delete": "14385supprimer",
    # No real emoji exists yet for "ban" or "waiting" in the portal list —
    # they'll just use their unicode defaults (🚫 / ⏳) until one is added.
}
...
class VoiceControlView(discord.ui.View):
    ...
    def __init__(self):
        ...
        # Try to upgrade each button's unicode default to the matching real
        # Estrada custom emoji, now that the bot is running and Application
        # Emojis may be loaded. Falls back to the safe unicode default
        # (already set by the @discord.ui.button decorator) if not found.
        for button_attr, app_name in _APP_EMOJI_NAMES.items():
            button = getattr(self, f"btn_{button_attr}", None)
            if button is None:
                continue
            emoji = emoji_loader.get_obj(app_name)
            if emoji:
                button.emoji = emoji
...
def build_panel_embed(guild: discord.Guild, cfg: dict) -> discord.Embed:
    ...
    embed.set_footer(text=cfg.get("panel_footer") or f"{config.SERVER_NAME}")
    return embed
