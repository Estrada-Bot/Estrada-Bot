...
def _resolve_emoji(raw):
    """Let staff type a Estrada emoji name (e.g. 'fl_locked') into
    /ticket customize instead of having to paste the full <:name:id> tag.
    If `raw` matches a loaded Application Emoji name, use the real emoji.
    Otherwise pass it through unchanged (so plain unicode like 🔒, or an
    already-pasted <:name:id> tag, still works as before)."""
    if not raw:
        return raw
    return emoji_loader.get_obj(raw) or raw
...
class TicketCreateView(discord.ui.LayoutView):
    """Components V2 card — one combined message (title + description +
    optional banner + footer/button), same look as the apply_system panels.
    NOTE: constructor is now TicketCreateView(cfg) — if you re-register this
    as a persistent view on startup, pass the guild's stored cfg too:
    TicketCreateView(ts.get(str(guild.id), {}))"""
    ...
    def __init__(self, cfg: dict = None):
        ...
        title = cfg.get("panel_title") or "🎫 Support Tickets"
        message = cfg.get("panel_message") or "Our support team will be with you shortly."
        banner_url = cfg.get("banner_url") or ""
        ...
        button = discord.ui.Button(
            ...
            emoji=_resolve_emoji(btn["open_emoji"]) if btn["open_emoji"] else None,
            style=BUTTON_STYLES.get((btn.get("open_style") or "blue").lower(), discord.ButtonStyle.primary),
            custom_id="ticket_open",
        )
        button.callback = self.open_ticket

        items = [discord.ui.TextDisplay(intro_text)]
        if banner_url:
            items.append(discord.ui.Separator())
            items.append(discord.ui.MediaGallery(discord.MediaGalleryItem(banner_url)))
        items.append(discord.ui.Separator())
        # Section = text on the left, button lined up on the right, same
        # row — matches the apply_system panel layout exactly.
        items.append(discord.ui.Section(discord.ui.TextDisplay(footer_text), accessory=button))
...
class TicketControlView(discord.ui.View):
    def __init__(self):
        ...
        self.claim.emoji = _resolve_emoji(btn["claim_emoji"])
        self.close.label = btn["close_label"]
        self.close.emoji = _resolve_emoji(btn["close_emoji"])
...
class Tickets(commands.Cog):
    ...
    @app_commands.describe(
        ...
        footer_text="Message shown next to the Open Ticket button (optional)",
    )
    async def ticket_setup(
        ...
        panel_view = TicketCreateView(cfg)
        try:
            await panel_channel.send(view=panel_view)
            # Re-register as persistent so the button keeps working after a restart.
    ...
    @app_commands.describe(
        ...
        footer_text="New message shown next to the Open Ticket button (optional)",
    )
    ...
    @app_commands.describe(
        ...
        open_style="Open-ticket button color: grey / blue / green / red (optional)",
    ...
    async def ticket_customize(
        self,
        interaction: discord.Interaction,
        open_label: str = None, open_emoji: str = None, open_style: str = None,
        claim_label: str = None, claim_emoji: str = None,
        close_label: str = None, close_emoji: str = None,
        instructions_text: str = None,
    ):
        if not any([open_label, open_emoji, open_style, claim_label, claim_emoji, close_label, close_emoji, instructions_text]):
        ...
        if open_style:
            style = open_style.strip().lower()
            btn["open_style"] = style if style in BUTTON_STYLES else "blue"
